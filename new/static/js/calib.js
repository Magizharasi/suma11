...
class CalibrationManager {
    ...
    redirectToChallenge() {
        // --- UPDATED: Signal the main application to start the background worker ---
        console.log('Calibration complete. Sending signal to start background worker...');
        
        fetch('/api/start_worker', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_id: this.userId }) // Optionally send user data
        })
        .then(response => response.json())
        .then(data => {
            console.log('Start worker signal response:', data.message);
            // Redirect to the dashboard AFTER the signal has been sent
            window.location.href = '/challenge';
        })
        .catch(error => {
            console.error('Failed to send start worker signal:', error);
            // Still redirect even if the signal fails, but log the error
            alert('Could not start background monitoring. Please restart the application.');
            window.location.href = '/challenge';
        });
    }
