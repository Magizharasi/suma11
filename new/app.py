...
def create_app(config_name='development'):
    ...
    def start_worker():
        """Endpoint to signal the main application to start the background worker."""
        from flask import current_app
    
        if hasattr(current_app, 'trigger_start_worker'):
            current_app.trigger_start_worker()
            logger.info("Signal sent to start background worker.")
            return jsonify({'status': 'success', 'message': 'Signal to start worker sent.'}), 200
        else:
            logger.error("Could not send signal to start worker.")
            return jsonify({'status': 'error', 'message': 'Worker trigger not found.'}), 500
    ...
    def logout():
        ...
        from flask import current_app # Import here to avoid circular dependency issues
        ...
        try:
            ...
            if session_id and session_id in active_sessions:
                ...
                if hasattr(current_app, 'trigger_shutdown'):
                    current_app.trigger_shutdown()
    ...
    @app.route('/api/background_data', methods=['POST'])
    def handle_background_data():
        """Receives and processes data from the background worker."""
        try:
            data = request.get_json()
            session_id = data.get('session_id')
            ...
            if not session_id or not user_id:
                return jsonify({'error': 'Missing session or user identification'}), 400

            # Authenticate the session
            session_data = authenticate_session(session_id)
            if not session_data or not session_data.get('calibration_complete', False):
                # --- MODIFIED: Added a return statement here ---
                return jsonify({'status': 'ignored', 'reason': 'Invalid session or calibration incomplete'}), 200

            # Initialize user components if not already loaded
            initialize_user_components(user_id)
            extractor = user_extractors[user_id]

            # Process and authenticate mouse data
            if mouse_data:
                mouse_features = extractor.extract_mouse_features(mouse_data)
                if mouse_features:
                    auth_result = perform_real_time_authentication(user_id, mouse_features, 'mouse')
                    logger.info(f"Background mouse auth for user {user_id}: Score {auth_result.get('authenticity_score')}")

            # Process and authenticate keystroke data
            if keyboard_data:
                key_features = extractor.extract_keystroke_features(keyboard_data)
                if key_features:
                    auth_result = perform_real_time_authentication(user_id, key_features, 'keystroke')
                    logger.info(f"Background keystroke auth for user {user_id}: Score {auth_result.get('authenticity_score')}")

            return jsonify({'status': 'success'}), 200

        except Exception as e:
            logger.error(f"Error handling background data: {e}")
            return jsonify({'error': 'Failed to process background data'}), 500
        ...
        try:
            ...
            if session_data.get('calibration_complete', False):
                try:
                    ...
                    emit('auth_result', auth_result, room=session_id) # Added room=session_id for targeted emit
                    ...
                    if auth_result.get('alert_level', 0) > 0:
                        ...
                        }, room=session_id) # Added room=session_id
                    ...
                    if auth_result.get('anomaly_detected', False):
                        try:
                            db_manager.log_auth_event(
                                ...
                                {
                                    'anomaly_score': auth_result.get('anomaly_score', 0),
                                    'confidence': auth_result.get('confidence', 0),
                                    'data_type': data_type
                                },
                                request.remote_addr
    ...
    def perform_real_time_authentication(user_id: int, features: Dict, data_type: str) -> Dict:
        ...
        try:
            if not user_models.get(user_id) or not user_extractors.get(user_id):
                initialize_user_components(user_id)
            
            extractor = user_extractors[user_id]
            model_ensemble = user_models[user_id]
            drift_detector = user_drift_detectors[user_id]
        
            # Ensure the single feature set has the correct dimensions
            # This is now a single dictionary, not a list of features
            fixed_features = extractor.fix_feature_dimensions(features)

            # Get ensemble prediction
            # The ensemble model expects a list, so we wrap our single feature dict in a list
            ensemble_result = user_models[user_id].predict_ensemble([fixed_features])
        
            auth_score = ensemble_result['ensemble']['authenticity_score']
            confidence = ensemble_result['ensemble']['confidence']
            anomaly_score = max(0.0, 1.0 - auth_score)

            # --- 1. Drift Detection Logic (ADDED) ---
            drift_detector.add_sample(fixed_features, data_type)
            drift_analysis = drift_detector.get_drift_analysis()
            if drift_analysis.get('drift_detected'):
                logger.info(f"Behavioral drift detected for user {user_id}. Score: {drift_analysis.get('drift_score')}")
                # Lower the authentication score slightly if drift is detected
                auth_score *= 0.95

            # --- 2. Model Updating Logic (ADDED) ---
            # Decide if the current behavior is genuine enough to update the models
            is_genuine_for_update = auth_score > 0.7 and confidence > 0.6
            if is_genuine_for_update:
                model_ensemble.update_models(fixed_features, is_genuine=True)
                logger.debug(f"Online models updated for user {user_id} with genuine data.")

            # Determine alert level
            alert_level = 0
            alert_message = "Normal behavior"
            if anomaly_score > app.config.get('ANOMALY_SCORE_THRESHOLD', 0.8):
                alert_level = 2
                alert_message = "High-risk behavior detected"
            elif anomaly_score > 0.5:
                alert_level = 1
                alert_message = "Unusual behavior detected"
            ...
            logger.error(f"Real-time authentication error for user {user_id}: {str(e)}")
            traceback.print_exc()
            return {
                ...
                'anomaly_score': 0.5,
                'alert_level': 0,
                'alert_message': 'Error during analysis'
            }
...
if __name__ == '__main__':
    ...
    logger.info("Starting Behavioral Authentication System with Eventlet...")
